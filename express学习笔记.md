#Express简介
> 是一个简洁、灵活的node.js Web应用开发框架,是目前最流行的基于Node.js的Web开发框架. 它提供一系列强大的功能，比如：
*  模板解析
*  静态文件服务
*  中间件
*  路由控制
...
###简单创建一个express
首先需要提供一个工厂函数，创建app，之后的多项功能将在该app对象上添加
```javascript
...
###路由实现
express一个最重要的功能即是定义路由规则，通过对这些规则的匹配返回不同的内容
首先我们需要一种叫`router`(路由容器)的东西来存放具体的`route`
